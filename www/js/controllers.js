angular
	...
	.controller('EmotionsCtrl', function(
		$scope,
		$http,
		emotionApiService,
		$firebaseArray,
		...
		$scope.carregarFoto = function() {
			...
			if ($scope.dados.url == '') {
				...
				$http({
					method: 'POST',
					url:
						'https://brazilsouth.api.cognitive.microsoft.com/face/v1.0/detect?returnFaceAttributes=emotion',
					headers: {
					...
					function(respostaErro) {
						alert(
							'Houve um erro na leitura de sua foto, por favor tente novamente ou envie outra foto.'
						)
					}
				)
			}
		...
		$scope.encontrarSentimento = function() {
			...
			$scope.sentimentos = [
				...
				},
				{
					...
					sugestao:
						'Para diminuir o desprezo sentido tente as seguintes sugestões:',
					sugestao1: 'Pense em coisas positivas',
					sugestao2: 'Tente usar empatia',
					sugestao3:
						'Pense que a situação do outro lado possa ser mais complicada do que você imagina'
				},
				{
					sentimento: 'com nojo',
					valor: resultado.faceAttributes.emotion.disgust,
					sugestao:
						'Para miminizar o sentimento de nojo tente as seguintes sugetões:',
				...
				},
				{
					...
					sugestao3:
						'Fique calmo que tudo vai dar certo se você desejar e se empenhar para isso'
				},
				{
					sentimento: 'feliz',
